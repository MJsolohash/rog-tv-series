  ...
  const fetchRecentSeries = async () => {
    try {
      ...
      if (data.success && data.series) {
        ...
        // Sort by createdAt (newest first) if available, otherwise use reverse order
        const recent = [...allSeries].reverse().slice(0, 50);
        setSeries(recent);
      } else {
        setSeries([]);
      }
      ...
      setError('Failed to load recent series');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-gray-400 text-sm tracking-wider">LOADING RECENT SERIES...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-black pt-24 px-4">
      <div className="container mx-auto">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-white">Recently Added</h1>
          <div className="w-16 h-0.5 bg-red-500 mt-2 rounded-full"></div>
          <p className="text-gray-400 mt-2">{series.length} recently added series</p>
        </div>

        {/* Series Grid */}
        {series.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500">No recent series available</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-5">
            {series.map((item, idx) => (
              <RecentCard key={item._id} series={item} index={idx} />
            ))}
          </div>
        )}
      </div>
...
const RecentCard = ({ series, index = 0 }) => {
